...
export const PhotoboothStripCard: React.FC<PhotoboothStripCardProps> = ({
  ...
  const layoutType: PhotoboothLayoutType =
    frame.layoutType || template?.layoutType || (frame.shotCount === 4 ? (isHorizontal ? 'wide_duo_4cut' : 'strip_4') : frame.shotCount === 2 ? (isHorizontal ? 'wide_duo_2cut' : 'strip_2') : frame.shotCount === 1 ? 'polaroid_square' : isHorizontal ? 'grid_2x2' : 'strip_3');
  ...
  // Container sizing based on layout type & orientation
  let cardWidthClass = 'w-[290px] sm:w-[320px] p-4 py-5';

  if (layoutType === 'wide_duo_2cut') {
    // Wide and shorter card, 2 photos side by side
    cardWidthClass = 'w-[360px] sm:w-[410px] p-4 py-4.5';
  } else if (layoutType === 'wide_duo_4cut') {
    // Wide compact card, 4 photos (2x2)
    cardWidthClass = 'w-[350px] sm:w-[390px] p-4 py-4.5';
    ...
    cardWidthClass = 'w-[300px] sm:w-[330px] p-5 pb-10';
  } else if (isHorizontal) {
    // General horizontal strip
    cardWidthClass = 'w-[420px] sm:w-[480px] p-4 py-4.5';
  }
        ...
        <div className="absolute inset-[3px] bg-stone-100 flex items-center justify-center overflow-hidden rounded-xs">
          {photo ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center p-3 text-center bg-stone-50/80">
              <span style={{ color: effectiveAccent }} className="text-base mb-0.5">🎁</span>
              <span className="text-[9px] font-bold opacity-85 leading-tight">هدية عند الاكتمال</span>
              <span className="text-[7px] text-stone-400 mt-0.5">#{slotFormatted}</span>
            </div>
          ) : (
            <div className="w-full h-full flex flex-col items-center justify-center bg-black/[0.02] text-stone-400">
              ...
              <span className="text-[8px] font-medium mt-0.5 opacity-60">في الانتظار</span>
            </div>
          )}
        </div>
    ...
    <div className={`flex flex-col items-center ${className}`}>
      ...
      >
        ...
        <div className="relative z-10 w-full my-1">
          {/* A. Wide Duo: 2 Photos Side by Side */}
          ...
          {/* H. Horizontal Strip (if orientation is horizontal for 2, 3, or 4 cuts) */}
          ...
          {/* I. Standard Vertical Strips */}
          {!isHorizontal && (layoutType === 'strip_4' || layoutType === 'strip_3' || layoutType === 'strip_2' || layoutType === 'film_35mm') && (
            <div className="flex flex-col gap-2">
        ...
        <div
          style={{ borderColor: effectiveBorder }}
          className="relative z-10 mt-3.5 pt-1.5 border-t-[0.5px] flex items-center justify-between text-[8px] opacity-80 font-mono px-1"
        >
          ...
          <span className="tracking-widest uppercase font-bold">
            {template?.dimensions || (isHorizontal ? 'HORIZONTAL' : 'VERTICAL')}
          </span>
        </div>
