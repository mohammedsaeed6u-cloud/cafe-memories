    ...
    <div className="space-y-6">
      ...
      <div className="p-6 bg-white rounded-3xl border border-stone-200/90 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <label className="flex items-center gap-2 px-3 py-2 rounded-xl bg-stone-50 border border-stone-200 text-xs font-bold text-stone-700 cursor-pointer hover:bg-stone-100 transition">
            <input
              type="checkbox"
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-stone-100 hover:bg-stone-200 text-stone-700 font-bold text-xs border border-stone-200 transition flex items-center gap-1.5 cursor-pointer"
          >
      ...
      {batchActionMsg && (
        <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-xs font-bold animate-in fade-in shadow-xs flex items-center gap-2">
      ...
      {isLoadingMemories ? (
        ...
        <div className="p-12 rounded-3xl bg-white border border-stone-200/90 text-center shadow-xs space-y-2">
          ...
          <h4 className="font-bold text-sm text-stone-700">لا توجد صور في هذا التصنيف</h4>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredMemories.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className={`p-3.5 rounded-2xl bg-white border-2 transition flex flex-col justify-between gap-3 shadow-xs hover:shadow-sm ${
                  isApproved
              ...
              >
                <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-stone-100 border border-stone-200 group">
                  {m.originalUrl ? (
                  ...
                  <button
                    ...
                    className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-white text-xs font-bold gap-1.5 cursor-pointer backdrop-blur-2xs"
                  >
                ...
                <div className="space-y-1">
                  <h4 className="font-black text-xs text-stone-900 truncate">{m.customerName}</h4>
                  {m.caption && (
                ...
                <div className="flex items-center gap-1.5 pt-2 border-t border-stone-100">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-xl border border-stone-200 hover:bg-rose-50 hover:border-rose-200 hover:text-rose-600 text-stone-400 transition cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl animate-in zoom-in-95">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-sm text-stone-900">{previewMemory.customerName}</h3>
              <button
                type="button"
            ...
            <button
              type="button"
              onClick={() => setPreviewMemory(null)}
              className="w-full py-2.5 rounded-xl bg-stone-900 text-white font-bold text-xs cursor-pointer hover:bg-black transition"
            >
              إغلاق
            </button>
          </div>
