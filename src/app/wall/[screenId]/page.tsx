...
import { Coffee, QrCode, Maximize2, Sparkles, Heart, Clock, ArrowLeft, Wifi } from 'lucide-react';
...
export default function LiveWallPage({ params }: PageProps) {
  ...
  // Fetch Live Memories from Supabase + Subscribe to Realtime
  useEffect(() => {
    ...
    const fetchApprovedMemories = async () => {
      try {
        ...
        console.warn('Live Wall Supabase fetch fallback:', err);
      }
    };

    fetchApprovedMemories();

    // Subscribe to Realtime changes on memories table
  ...
  // Slide Rotation Timer (every 7 seconds)
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setIsCtaSlide(prev => {
        if (!prev) {
          return true; // Show CTA slide every other turn
        } else {
    ...
    <div className="relative w-screen h-screen bg-stone-950 text-white overflow-hidden flex flex-col font-sans select-none">
      ...
      <header className="absolute top-0 inset-x-0 z-30 px-8 py-5 flex items-center justify-between bg-gradient-to-b from-black/90 via-black/50 to-transparent">
        <div className="flex items-center gap-3.5">
          <Link
            href="/"
            className="w-10 h-10 rounded-xl bg-stone-900/80 hover:bg-stone-800 border border-stone-800 flex items-center justify-center text-stone-400 hover:text-white transition-colors"
          >
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-xs font-semibold text-amber-300">
            <Sparkles className="w-3.5 h-3.5 text-amber-400" />
            <span>Guest Moments Wall</span>
          </div>
